...
ids <- idsFormatted[idsFormatted %in% colnames(t)]
...
alleleChecker <- function(x,ref='REF',alt='ALT',eff='Effect.Allele',nonEff='Other.Allele'){
    ...
    #4: REF==strandFlip(nonEff) && ALT==strandFlip(eff) -> Do nothing
...
#Flip those which have cases 1 or 3 and remove those with NA cases
...
GRStypes <- read.table('GRS.spec',h=F,as.is=T)
...
#Final formatting
write.table(finalGRS,'GRS_collected.readyForPlink',row.names=F,quote=F)
